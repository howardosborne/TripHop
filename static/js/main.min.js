var map,sidePanel,popup,settings,startSelect,destinationSelect,inspiredTrips,agencyLookup,journeyLookup,freestyleStartPoints,possibleHops,candidateHop,hops,routeLines,possibleInspiredTrip,possibleInspiredTripRouteLines,possibleFromToStartPoints,possibleFromToEndPoints,fromToStartPoint,fromToDestination,fromToLines,liveRouteLines,liveStops,dbServer="v5.db.transport.rest",all_places={},all_hops={},trips={},stopsPlacesLookup={},stopsForPlaces={},lastScrollTop={},lastTab="tab-home",fromToStops={};function startUp(){map=L.map("map").setView([45,10],5);L.tileLayer("https://tile.openstreetmap.org/{z}/{x}/{y}.png",{maxZoom:19,attribution:'&copy; <a href="http://www.openstreetmap.org/copyright">OpenStreetMap</a>'}).addTo(map);L.control.scale({position:"topleft"}).addTo(map),L.control.zoom({position:"bottomright"}).addTo(map),sidePanel=L.control.sidepanel("mySidepanelLeft",{tabsPosition:"top",startTab:"tab-home"}).addTo(map),freestyleStartPoints=L.markerClusterGroup({maxClusterRadius:40}),possibleHops=L.markerClusterGroup({maxClusterRadius:40}),hops=new L.LayerGroup,routeLines=new L.LayerGroup,possibleFromToStartPoints=L.markerClusterGroup({maxClusterRadius:40}),possibleFromToEndPoints=L.markerClusterGroup({maxClusterRadius:40}),fromToStartPoint=new L.LayerGroup,fromToDestination=new L.LayerGroup,fromToLines=new L.LayerGroup,possibleInspiredTrip=new L.LayerGroup,possibleInspiredTripRouteLines=new L.LayerGroup,liveStops=L.markerClusterGroup({maxClusterRadius:40}),liveRouteLines=new L.LayerGroup,L.easyButton('<img src="/static/icons/resize.png" alt="resize" title="resize">',function(e,t){t.fitBounds(possibleHops.getBounds())}).addTo(map),getSettings(),getAllPlaces(),getAgencyLookup(),getInspiredTrips(),showHomeTab()}function getSettings(){href=encodeURIComponent(window.location.href),url="https://script.google.com/macros/s/AKfycbyEskUlQxAOp1rXvo40xbyZDQEgiojWiZXBexBGCLyr0ptkz2kT-3vjvXcCwzTH-zPSGg/exec?request=settings&href="+href;var e=new XMLHttpRequest;e.onreadystatechange=function(){4==this.readyState&&200==this.status&&(settings=JSON.parse(this.responseText))},e.open("GET",url,!0),e.send()}function getAllPlaces(){var e=new XMLHttpRequest;e.onreadystatechange=function(){4==this.readyState&&200==this.status&&(all_places=JSON.parse(this.responseText),getStopsForPlaces())},e.open("GET","/static/places.json",!0),e.send()}function getStopsForPlaces(){var e=new XMLHttpRequest;e.onreadystatechange=function(){4==this.readyState&&200==this.status&&(stopsForPlaces=JSON.parse(this.responseText),getAllHopsandShowPlaceMarkers())},e.open("GET","/static/stops_for_places.json",!0),e.send()}function getAllHopsandShowPlaceMarkers(){var e=new XMLHttpRequest;e.onreadystatechange=function(){var e;4==this.readyState&&200==this.status&&(e=JSON.parse(this.responseText),all_hops=e,addFreestyleStartPoints(),checkHref())},e.open("GET","/static/hops.json",!0),e.send()}function addFreestyleStartPoints(){Object.entries(all_places).forEach(e=>{var[e,t]=e;e in all_hops&&(e=L.icon({iconUrl:"/static/icons/home.png",iconSize:[36,36],iconAnchor:[18,36]}),(e=L.marker([t.place_lat,t.place_lon],{icon:e})).bindTooltip(decodeURI(t.place_name)),e.properties=t,e.addEventListener("click",_starterMarkerOnClick),e.addTo(freestyleStartPoints))})}function checkHref(){var e=window.location.href,t=RegExp(".+action=fromto&from=(\\w+)&to=(\\w+)","g"),a=RegExp(".+action=place&place_id=(\\w+)","g"),o=RegExp(".+action=inspire&id=(\\w+)","g");RegExp(".+action=depart&id=(\\w+)","g");let s;if(s=t.exec(e))fromToStops[stopsForPlaces[s[1]][0].name]={lat:stopsForPlaces[s[1]][0].location.latitude,lng:stopsForPlaces[s[1]][0].location.longitude,stationId:stopsForPlaces[s[1]][0].id},fromToStops[stopsForPlaces[s[2]][0].name]={lat:stopsForPlaces[s[2]][0].location.latitude,lng:stopsForPlaces[s[2]][0].location.longitude,stationId:stopsForPlaces[s[2]][0].id},showDestinationTab(),document.getElementById("startSelect").value=stopsForPlaces[s[1]][0].name,document.getElementById("destinationSelect").value=stopsForPlaces[s[2]][0].name,findFabRoutes();else if(s=a.exec(e)){let e=s[1];e=e.replace("germany","Germany");t=all_places[e],a=get_place_details_block(e);document.getElementById("place_body").innerHTML=a,popup_text=`
      ...
      </div>`,popup=L.popup().setLatLng([t.place_lat,t.place_lon]).setContent(popup_text).openOn(map)}(s=o.exec(e))&&(showInspireTab(),showInspiredRoute(s[1]))}function getStopsNearLocation(e,t){e=`https://${dbServer}/stops/nearby?latitude=${e}&longitude=${t}&results=1&distance=10000&stops=true`,t=new XMLHttpRequest;t.onreadystatechange=function(){if(4==this.readyState)if(200==this.status){var t=JSON.parse(this.responseText);console.log("processing start stops"),console.log(this.responseText);let e="";for(var a=0;a<t.length;a++){t[a];fromToStops[t[a].name]={lat:t[a].location.latitude,lng:t[a].location.longitude,stationId:t[a].id},e+=`<option>${t[a].name}</option>`}document.getElementById("startList").innerHTML=e}else console.log(`Status: ${this.status} 
 Response text: `+this.responseText),document.getElementById("fromToResults").innerHTML="hmm, something went wrong... maybe time to put the kettle on"},t.open("GET",e,!0),t.send()}function getStartStops(){var e=document.getElementById("startSelect").value,e=`https://${dbServer}/locations?query=${encodeURIComponent(e)}&poi=false&addresses=false`,t=new XMLHttpRequest;t.onreadystatechange=function(){if(4==this.readyState)if(200==this.status){var t=JSON.parse(this.responseText);console.log("processing start stops"),console.log(this.responseText);let e="";for(var a=0;a<t.length;a++){t[a];fromToStops[t[a].name]={lat:t[a].location.latitude,lng:t[a].location.longitude,stationId:t[a].id},e+=`<option>${t[a].name}</option>`}document.getElementById("startList").innerHTML=e}else console.log(`Status: ${this.status} 
 Response text: `+this.responseText),document.getElementById("fromToResults").innerHTML="hmm, something went wrong... maybe time to put the kettle on"},t.open("GET",e,!0),t.send()}function getDestinationStops(){var e=document.getElementById("destinationSelect").value,e=`https://${dbServer}/locations?query=${encodeURIComponent(e)}&poi=false&addresses=false`,t=new XMLHttpRequest;t.onreadystatechange=function(){if(4==this.readyState)if(200==this.status){var t=JSON.parse(this.responseText);console.log("processing start stops"),console.log(this.responseText);let e="";for(var a=0;a<t.length;a++)fromToStops[t[a].name]={lat:t[a].location.latitude,lng:t[a].location.longitude,stationId:t[a].id},e+=`<option lat="${t[a].location.latitude}" lng="${t[a].location.longitude}" stationId="${t[a].id}">${t[a].name}</option>`;document.getElementById("destinationList").innerHTML=e}else console.log(`Status: ${this.status} 
 Response text: `+this.responseText),document.getElementById("fromToResults").innerHTML="hmm, something went wrong... maybe time to put the kettle on"},t.open("GET",e,!0),t.send()}function findFabRoutes(){popup&&popup.close();var e=fromToStops[document.getElementById("startSelect").value],t=fromToStops[document.getElementById("destinationSelect").value],a=(map.hasLayer(fromToStartPoint)||map.addLayer(fromToStartPoint),map.hasLayer(fromToDestination)||map.addLayer(fromToDestination),map.hasLayer(fromToLines)||map.addLayer(fromToLines),fromToStartPoint.clearLayers(),fromToDestination.clearLayers(),fromToLines.clearLayers(),document.getElementById("fromToResults").innerHTML="",L.icon({iconUrl:"/static/icons/home.png",iconSize:[36,36],iconAnchor:[18,36]}));let o=L.marker([e.lat,e.lng],{icon:a});o.properties=e,o.bindTooltip(document.getElementById("startSelect").value),o.addTo(fromToStartPoint),a=L.icon({iconUrl:"/static/icons/destination.png",iconSize:[36,36],iconAnchor:[18,36]}),(o=L.marker([t.lat,t.lng],{icon:a})).properties=t,o.bindTooltip(document.getElementById("destinationSelect").value),o.addTo(fromToDestination),getJourneysWithoutDuplicates(e.stationId,t.stationId)}function getLiveStops(){var e=document.getElementById("liveSelect").value,e=`https://${dbServer}/locations?query=${encodeURIComponent(e)}&poi=false&addresses=false`,t=new XMLHttpRequest;t.onreadystatechange=function(){if(4==this.readyState)if(200==this.status){var t=JSON.parse(this.responseText);console.log("processing start stops"),console.log(this.responseText);let e="";for(var a=0;a<t.length;a++){t[a];fromToStops[t[a].name]={lat:t[a].location.latitude,lng:t[a].location.longitude,stationId:t[a].id},e+=`<option>${t[a].name}</option>`}document.getElementById("liveList").innerHTML=e}else console.log(`Status: ${this.status} 
 Response text: `+this.responseText),document.getElementById("routes_from_places").innerHTML="hmm, something went wrong... maybe time to put the kettle on"},t.open("GET",e,!0),t.send()}function getLiveDepartures(){document.getElementById("routes_from_places").innerHTML="";var e=`<h5>Departures from ${document.getElementById("liveSelect").value}</h5>`;document.getElementById("routes_from_places").insertAdjacentHTML("beforeend",e),getDepartures(fromToStops[document.getElementById("liveSelect").value].stationId)}function addPossibleFromToStartPoints(){Object.entries(all_places).forEach(e=>{var[e,t]=e;e in stopsForPlaces&&0<stopsForPlaces[e].length&&(e=L.icon({iconUrl:"/static/icons/home.png",iconSize:[36,36],iconAnchor:[18,36]}),(e=L.marker([t.place_lat,t.place_lon],{icon:e})).bindTooltip(decodeURI(t.place_name)),e.properties=t,e.addEventListener("click",_fromMarkerOnClick),e.addTo(possibleFromToStartPoints))})}function addDestinationMarkers(){Object.entries(all_places).forEach(e=>{var[e,t]=e;e in stopsForPlaces&&0<stopsForPlaces[e].length&&(e=L.icon({iconUrl:"/static/icons/destination.png",iconSize:[36,36],iconAnchor:[18,36]}),(e=L.marker([t.place_lat,t.place_lon],{icon:e})).bindTooltip(decodeURI(t.place_name)),e.properties=t,e.addEventListener("click",_destinationMarkerOnClick),e.addTo(possibleFromToEndPoints))});var e=window.location.href,t=RegExp(".+action=fromto&from=(\\w+)&to=(\\w+)","g"),a=RegExp(".+action=place&place_id=(\\w+)","g"),o=RegExp(".+action=inspire&id=(\\w+)","g");let s;if(s=t.exec(e))showDestinationTab(),startSelect.setSelected(s[1]),destinationSelect.setSelected(s[2]),findFabRoutes();else if(s=a.exec(e)){let e=s[1];e=e.replace("germany","Germany");t=all_places[e],a=get_place_details_block(e);document.getElementById("place_body").innerHTML=a,popup_text=`
        <div class="card mb-3">
         <img src="${t.place_image}" class="img-fluid rounded-start" style="max-height:250px" alt="${t.place_name}" title = "${t.image_attribution}">
         <div class="card-img-overlay">
         <div class="row justify-content-evenly"><div class="col"><a href="#" class="h3" style="font-family: 'Cantora One', Arial; font-weight: 700; vertical-align: baseline; color:white; text-shadow:-1px 1px 0 #000, 1px 1px 0 #000; " onclick="openPlaceDetails('${t.place_id}')">${t.place_name}</a></div></div>
  ...
  `,popup=L.popup().setLatLng([place.place_lat,place.place_lon]).setContent(popup_text).openOn(map)}function clearAllLayers(){hops.clearLayers(),fromToStartPoint.clearLayers(),fromToDestination.clearLayers(),routeLines.clearLayers(),possibleInspiredTrip.clearLayers(),possibleInspiredTripRouteLines.clearLayers(),possibleHops.clearLayers(),freestyleStartPoints.clearLayers(),possibleFromToStartPoints.clearLayers(),possibleFromToEndPoints.clearLayers(),liveRouteLines.clearLayers(),liveStops.clearLayers()}function toRadians(e){return e*(Math.PI/180)}function distanceBetweenTwoPoints(e,t,o,s){return from_lat_rad=toRadians(parseFloat(e)),from_lon_rad=toRadians(parseFloat(t)),to_lat_rad=toRadians(parseFloat(o)),to_lon_rad=toRadians(parseFloat(s)),dist_lon=to_lon_rad-from_lon_rad,dist_lat=to_lat_rad-from_lat_rad,a=Math.sin(dist_lat/2)**2+Math.cos(from_lat_rad)*Math.cos(to_lat_rad)*Math.sin(dist_lon/2)**2,c=2*Math.atan2(Math.sqrt(a),Math.sqrt(1-a)),R=6373,distance=R*c}function findFromStops(o,s){let i=[];var e=all_places[o],e=`https://${dbServer}/stops/nearby?latitude=${e.place_lat}&longitude=${e.place_lon}&results=1&distance=${e.lat_lon_tolerance}000&stops=true`,t=new XMLHttpRequest;t.onreadystatechange=function(){if(4==this.readyState)if(200==this.status){for(var e=JSON.parse(this.responseText),t=0;t<e.length;t++){var a=e[t];"stop"==a.type&&(stopsPlacesLookup[a.id]=o,console.log(`findFromStops: ${a.id} - `+a.name),i.push(a.id))}findToStops(o,s,i[0])}else console.log(`Status: ${this.status} 
 Response text: `+this.responseText),document.getElementById("fromToResults").innerHTML="hmm, something went wrong... maybe time to put the kettle on"},t.open("GET",e,!0),t.send()}function findToStops(o,s,i){let n=[];var e=all_places[s],e=`https://${dbServer}/stops/nearby?latitude=${e.place_lat}&longitude=${e.place_lon}&results=1&distance=${e.lat_lon_tolerance}000&stops=true`,t=new XMLHttpRequest;t.onreadystatechange=function(){if(4==this.readyState)if(200==this.status){for(var e=JSON.parse(this.responseText),t=0;t<e.length;t++){var a=e[t];"stop"==a.type&&(stopsPlacesLookup[a.id]=s,console.log(`findToStops: ${a.id} - `+a.name),n.push(a.id))}getJourneysByStop(o,s,i,n[0])}else console.log(`Status: ${this.status} 
 Response text: `+this.responseText),document.getElementById("fromToResults").innerHTML="hmm, something went wrong... maybe time to put the kettle on"},t.open("GET",e,!0),t.send()}function getFromTo(p,c,d,m){trips={};var e=`https://${dbServer}/journeys?from=${d}&to=${m}&results=3&stopovers=false&transferTime=0&bike=false&startWithWalking=true&walkingSpeed=normal&tickets=false&polylines=false&subStops=true&entrances=true&remarks=true&scheduledDays=false&language=en&firstClass=false`,t=new XMLHttpRequest;t.onreadystatechange=function(){if(4==this.readyState)if(200==this.status){var e=JSON.parse(this.responseText),t=(console.log("processing journeys"),e.journeys),a=(journeyLookup={journeyCount:t.length,lastTripId:null},[]);if(0==t.length)document.getElementById("fromToResults").innerHTML="No results found";else for(var o=0;o<t.length;o++){var s=t[o].legs;let e="";for(var i,n=0;n<s.length;n++)"line"in s[n]&&(i=s[n].line.name,e+=i);if(a.includes(e))console.log("already included this journey");else{console.log("new journey "+e),document.getElementById("fromToResults").insertAdjacentHTML("beforeend",`<div id="${d}_${m}_${o}"></div>`),document.getElementById(d+`_${m}_`+o).insertAdjacentHTML("beforeend",`<h5>Option ${o+1}</h5>`);for(var l,r,n=0;n<s.length;n++)placeNear(s[n].destination.location.latitude,s[n].destination.location.longitude,p)?console.log(s[n].destination.name+` ${p} haven't left start`):placeNear(s[n].origin.location.latitude,s[n].origin.location.longitude,c)?console.log("already reached destination"):"line"in s[n]&&(l=s[n].line.name,e+=l,r=s[n].tripId,l)&&(document.getElementById(d+`_${m}_`+o).insertAdjacentHTML("beforeend",`<div id="${s[n].origin.id}_${s[n].destination.id}_${o}"></div>`),getTripsForLine(s[n].origin.id,s[n].destination.id,r,l,`${s[n].origin.id}_${s[n].destination.id}_`+o),journeyLookup.lastTripId=r)}}}else console.log(`Status: ${this.status} 
 Response text: `+this.responseText),document.getElementById("fromToResults").innerHTML="hmm, something went wrong... maybe time to put the kettle on"},t.open("GET",e,!0),t.send()}function getTripsForLine(c,d,m,e,u){var t=`https://${dbServer}/trips/${encodeURIComponent(m)}?lineName=`+encodeURIComponent(e),a=new XMLHttpRequest;a.onreadystatechange=function(){if(4==this.readyState)if(200==this.status){var o=JSON.parse(this.responseText);if(trips[encodeURIComponent(m)]=o,console.log(`processing trip from ${c} to `+d),console.log(e),"stopovers"in o){let s=o.stopovers,t="";if(o.remarks)for(let e=0;e<o.remarks.length;e++)t+="<br>"+o.remarks[e].text;let e="",a=!1,i,n;var r=[];let l=[];for(let o=0;o<s.length;o++){if(0!=o&&s[o].plannedArrival?s[o].timestamp=s[o].plannedArrival:s[o].timestamp=s[o].plannedDeparture,s[o].stop.id==c&&(a=!0,i=o),a){let t="",a=`showPlaceOnMap('${s[o].stop.location.latitude}', '${s[o].stop.location.longitude}','${s[o].stop.name},'${s[o].stop.id}')`;Object.entries(all_places).forEach(e=>{var[,e]=e;distanceBetweenTwoPoints(s[o].stop.location.latitude,s[o].stop.location.longitude,e.place_lat,e.place_lon)<=e.lat_lon_tolerance&&(a=`popupPlace('${e.place_id}')`,l.includes(e.place_id)||l.push(e.place_id),t='<span class="badge text-bg-light">Fab Hop!</span>')}),e+=`<li class="list-group-item"><a href="#" onclick="${a}">${s[o].stop.name} ${t}</a></li>`,r.push([s[o].stop.location.latitude,s[o].stop.location.longitude])}s[o].stop.id==d&&(n=o,a=!1)}badge=1<l.length?`<span class="badge text-bg-light">${l.length} fab hops!</span>`:1==l.length?'<span class="badge text-bg-light">1 fab hop!</span>':"";var p=`
            <div class="card">
              ...
              ${s[i].stop.name} to ${s[n].stop.name}
              </a> ${badge}
              </div>
 ...
 Response text: `+this.responseText),document.getElementById("fromToResults").innerHTML="hmm, something went wrong... maybe time to put the kettle on"},a.open("GET",t,!0),a.send()}async function getJourneysWithoutDuplicates(c,d){var a=[],e=await fetch(`https://${dbServer}/journeys?from=${c}&to=${d}&results=3&stopovers=false&transferTime=0&bike=false&startWithWalking=true&walkingSpeed=normal&tickets=false&polylines=false&subStops=true&entrances=true&remarks=true&scheduledDays=false&language=en&firstClass=false`);if(200==e.status){var o=(await e.json()).journeys;for(let e=0;e<o.length;e++){var s={from_stop_id:c,to_stop_id:d,legs:[]},i=o[e].legs;for(let a=0;a<i.length;a++)if("line"in i[a]){var n=i[a].line.name,l=i[a].tripId;if(l){l=`https://${dbServer}/trips/${encodeURIComponent(l)}?lineName=`+encodeURIComponent(n),n=await(await fetch(l)).json();if("stopovers"in n){var r={stops:[]},p=n.stopovers;let t=!1;for(let e=0;e<p.length;e++)0!=e&&p[e].plannedArrival?p[e].timestamp=p[e].plannedArrival:p[e].timestamp=p[e].plannedDeparture,p[e].stop.id==i[a].origin.id&&(t=!0,e),t&&r.stops.push({name:p[e].stop.name,latitude:p[e].stop.location.latitude,longitude:p[e].stop.location.longitude}),p[e].stop.id==i[a].destination.id&&(e,t=!1,s.legs.push(r))}}}var m=JSON.stringify(s);let t=!0;for(let e=0;e<a.length;e++)a[e]==m&&(t=!1,console.log("found duplicate footprint"));if(t){a.push(m),console.log(m);let r=a.length,p=(document.getElementById("fromToResults").insertAdjacentHTML("beforeend",`<div id="${c}_${d}_${r}"></div>`),document.getElementById(c+`_${d}_`+r).insertAdjacentHTML("beforeend",`<h5>Option ${r}</h5>`),0);s.legs.forEach(e=>{p++;var s=[];let i=[];var t=e.stops[0].name,a=e.stops[e.stops.length-1].name;let n="",l=e.stops;for(let o=0;o<l.length;o++){let t="",a=`showPlaceOnMap('${l[o].latitude}', '${l[o].longitude}','${l[o].name},'${l[o].id}')`;Object.entries(all_places).forEach(e=>{var[,e]=e;distanceBetweenTwoPoints(l[o].latitude,l[o].longitude,e.place_lat,e.place_lon)<=e.lat_lon_tolerance&&(a=`popupPlace('${e.place_id}')`,i.includes(e.place_id)||i.push(e.place_id),t='<span class="badge text-bg-light">Fab Hop!</span>')}),n+=`<li class="list-group-item"><a href="#" onclick="${a}">${l[o].name} ${t}</a></li>`,s.push([l[o].latitude,l[o].longitude])}badge=1<i.length?`<span class="badge text-bg-light">${i.length} fab hops!</span>`:1==i.length?'<span class="badge text-bg-light">1 fab hop!</span>':"";var o=`<div>
              ${`
              <div class="card">
                <div class="card-header">
                ...
                <ul class="list-group list-group-flush">
              `}${n}</ul></div></div>
              </div>`,o=(document.getElementById(c+`_${d}_`+r).insertAdjacentHTML("beforeend",o),L.polyline(s,{color:"#ff6600ff",weight:3,opacity:.5,smoothFactor:1}));o.bindTooltip(t+" to "+a),o.properties=e,o.addTo(fromToLines)})}}}}function getDepartures(o,e=1440){liveRouteLines.clearLayers(),map.hasLayer(liveRouteLines)||map.addLayer(liveRouteLines);var t=stopsPlacesLookup[o],t=(all_places[t],trips={},`https://${dbServer}/stops/${o}/departures?duration=`+e),e=new XMLHttpRequest;e.onreadystatechange=function(){if(4==this.readyState)if(200==this.status){var e=JSON.parse(this.responseText);console.log("processing departures"),console.log(this.responseText);for(var t=0;t<e.length;t++){var a=e[t];trip_id=a.tripId,line_name=a.line.name,getLiveTrips(o,trip_id,line_name)}}else console.log(`Status: ${this.status} 
 Response text: `+this.responseText),document.getElementById("fromToResults").innerHTML="hmm, something went wrong... maybe time to put the kettle on"},e.open("GET",t,!0),e.send()}function addDepartures(e,o,t=1440){map.hasLayer(liveRouteLines)||map.addLayer(liveRouteLines);var a=stopsPlacesLookup[o],a=(all_places[a],trips={},`https://${dbServer}/stops/${o}/departures?duration=`+t),t=new XMLHttpRequest;t.onreadystatechange=function(){if(4==this.readyState)if(200==this.status){var e=JSON.parse(this.responseText);console.log("processing departures"),console.log(this.responseText);for(var t=0;t<e.length;t++){var a=e[t];trip_id=a.tripId,line_name=a.line.name,getLiveTrips(o,trip_id,line_name)}}else console.log(`Status: ${this.status} 
 Response text: `+this.responseText),document.getElementById("fromToResults").innerHTML="hmm, something went wrong... maybe time to put the kettle on"},t.open("GET",a,!0),t.send()}function getLiveTrips(p,c,e){var e=`https://${dbServer}/trips/${encodeURIComponent(c)}?lineName=`+encodeURIComponent(e),t=new XMLHttpRequest;t.onreadystatechange=function(){if(4==this.readyState)if(200==this.status){var o=JSON.parse(this.responseText);if(trips[encodeURIComponent(c)]=o,console.log("processing trips"),console.log(this.responseText),"stopovers"in o){let s=o.stopovers,t="";if(o.remarks)for(let e=0;e<o.remarks.length;e++)t+="<br>"+o.remarks[e].text;let e="",a=!1,i;var l,r=[];let n=[];for(let o=0;o<s.length;o++){s[o].stop.id==p&&(a=!0,i=o);if(0!=o&&s[o].plannedArrival?s[o].timestamp=s[o].plannedArrival:s[o].timestamp=s[o].plannedDeparture,a){let t="",a=`showPlaceOnMap('${s[o].stop.location.latitude}', '${s[o].stop.location.longitude}','${s[o].stop.name}','${s[o].stop.id}')`;Object.entries(all_places).forEach(e=>{var[,e]=e;distanceBetweenTwoPoints(s[o].stop.location.latitude,s[o].stop.location.longitude,e.place_lat,e.place_lon)<=e.lat_lon_tolerance&&(a=`popupPlace('${e.place_id}')`,n.includes(e.place_id)||n.push(e.place_id),t='<span class="badge text-bg-light">Fab Hop!</span>')}),e+=`<li class="list-group-item">${s[o].timestamp.substring(11,19)}: <a href="#" onclick="showPlaceOnMap('${s[o].stop.location.latitude}', '${s[o].stop.location.longitude}','${s[o].stop.name}','${s[o].stop.id}')">${s[o].stop.name}</a></li>`,r.push([s[o].stop.location.latitude,s[o].stop.location.longitude])}}a&&(badge=1<n.length?`<span class="badge text-bg-light">${n.length} fab hops!</span>`:1==n.length?'<span class="badge text-bg-light">1 fab hop!</span>':"",l=`
            <div class="card">
            ...
            <ul class="list-group list-group-flush">
            `,document.getElementById("routes_from_places").insertAdjacentHTML("beforeend",l+e+"</ul></div></div></div>"),(l=L.polyline(r,{color:"#ff6600ff",weight:3,opacity:.5,smoothFactor:1})).bindTooltip(o.origin.name+" to "+o.destination.name),l.properties=o,l.addTo(liveRouteLines))}}else console.log(`Status: ${this.status} 
 Response text: `+this.responseText),document.getElementById("fromToResults").innerHTML="hmm, something went wrong... maybe time to put the kettle on"},t.open("GET",e,!0),t.send()}function getStopsFromTrips(p,c,e){var e=`https://${dbServer}/trips/${encodeURIComponent(c)}?lineName=`+encodeURIComponent(e),t=new XMLHttpRequest;t.onreadystatechange=function(){if(4==this.readyState)if(200==this.status){var o=JSON.parse(this.responseText);if(trips[encodeURIComponent(c)]=o,console.log("processing trips"),console.log(this.responseText),"stopovers"in o){let s=o.stopovers,t="";if(o.remarks)for(let e=0;e<o.remarks.length;e++)t+="<br>"+o.remarks[e].text;let e="",a=!1,i;var l,r=[];let n=[];for(let o=0;o<s.length;o++){s[o].stop.id==p&&(a=!0,i=o);if(0!=o&&s[o].plannedArrival?s[o].timestamp=s[o].plannedArrival:s[o].timestamp=s[o].plannedDeparture,a){let t="",a=`showPlaceOnMap('${s[o].stop.location.latitude}', '${s[o].stop.location.longitude}','${s[o].stop.name}','${s[o].stop.id}')`;Object.entries(all_places).forEach(e=>{var[,e]=e;distanceBetweenTwoPoints(s[o].stop.location.latitude,s[o].stop.location.longitude,e.place_lat,e.place_lon)<=e.lat_lon_tolerance&&(a=`popupPlace('${e.place_id}')`,n.includes(e.place_id)||n.push(e.place_id),t='<span class="badge text-bg-light">Fab Hop!</span>')}),e+=`<li class="list-group-item">${s[o].timestamp.substring(11,19)}: <a href="#" onclick="showPlaceOnMap('${s[o].stop.location.latitude}', '${s[o].stop.location.longitude}','${s[o].stop.name}','${s[o].stop.id}')">${s[o].stop.name}</a></li>`,r.push([s[o].stop.location.latitude,s[o].stop.location.longitude])}}a&&(badge=1<n.length?`<span class="badge text-bg-light">${n.length} fab hops!</span>`:1==n.length?'<span class="badge text-bg-light">1 fab hop!</span>':"",l=`
            <div class="card">
            <div class="card-header livetrip" onmouseover="showTripOnMap('${encodeURIComponent(c)}')" timestamp="${s[i].timestamp.substring(11,19)}">
            ${s[i].timestamp.substring(11,19)} 
            <a data-bs-toggle="collapse" href="#${encodeURIComponent(c)}" aria-expanded="false" aria-controls="${encodeURIComponent(c)}">
