...
from_to[["departure_time_x_hour","departure_time_x_minute","departure_time_x_second"]] = from_to["departure_time_x"].str.split(":",expand=True)
depart = pd.to_numeric(from_to["departure_time_x_hour"])*3600 + pd.to_numeric(from_to["departure_time_x_minute"])*60 + pd.to_numeric(from_to["departure_time_x_second"])
from_to[["arrival_time_y_hour","arrival_time_y_minute","arrival_time_y_second"]]  = from_to["arrival_time_y"].str.split(":")
arrival = pd.to_numeric(from_to["arrival_time_y_hour"])*3600 + pd.to_numeric(from_to["arrival_time_y_minute"])*60 + pd.to_numeric(from_to["arrival_time_y_second"])
...
wibble[["place_id","place_id_x","duration_min","duration_max","duration_median","place_id","place_name","stop_id","stop_name","stop_lat","stop_lon","place_brief_desc","place_longer_desc","place_image","place_tags","place_links"]].to_csv(from_to_places_file)
